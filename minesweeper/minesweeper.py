...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        neighbour=self.close_cells(cell)
        for c in neighbour:
            if c in self.mines:
                count_cpy-=1
            if c not in self.mines | self.safes:
                cells.add(c)
        new_sent=Sentence(cells,count_cpy)
    ...
    def check_knowledge(self):
        """ check for new safes and mines, updates knowledge if possible"""
    ...
    def make_random_move(self):
        ...
        """
        max_itr=self.height*self.width
        while max_itr:
            i,j=random.randrange(0,self.height),random.randrange(0,self.width)
